...
import Bot.Message.Common (MessageEnvT (runMessageEnvT), MessageHandlerEnv (MessageHandlerEnv, config, message), command, replyBack)
...
handleMessage
  message@Message
    ...
    isChatMessage' <- isChatMessage message
    -- liftIO $ putStrLn $ "isCommand" <> show isCommand'
    let mhe = MessageHandlerEnv {config, message, command = command'}
    ...
    res <-
      runThis $
        asum
          [ withCondition (isChatMessage' && isCommand') handleLawsuit,
